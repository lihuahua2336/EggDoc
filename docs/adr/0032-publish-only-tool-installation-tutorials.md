# Publish only tool installation tutorials

EggDoc currently publishes two tool installation tutorials: Codex and Claude Code. The primary navigation names this collection `工具教程`, and the home page, collection page, article navigation, related content, and search experience expose only these tutorials.

Lesson, note, and learning-path publishing capabilities and their routes are intentionally removed. Each published article should complete one installation task without mixing broader product education into the same page.
